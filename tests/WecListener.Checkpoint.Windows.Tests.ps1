...
function ReadListeners {
    @(Microsoft.WSMan.Management\Get-WSManInstance -ResourceURI 'winrm/config/Listener' -Enumerate -ErrorAction Stop|ForEach-Object {
...
try {
    ...
    foreach($listener in @($original|Where-Object Transport -eq 'HTTP')){
        ...
        Microsoft.WSMan.Management\Remove-WSManInstance -ResourceURI 'winrm/config/Listener' -SelectorSet @{Address=$listener.Address;Transport='HTTP'} -ErrorAction Stop
        $removed+=$listener
    }
    ...
    $created=$true
    $result=Microsoft.WSMan.Management\New-WSManInstance -ResourceURI 'winrm/config/Listener' -SelectorSet $selector -ValueSet $values -ErrorAction Stop
    ...
    try {$null=Microsoft.WSMan.Management\New-WSManInstance -ResourceURI 'winrm/config/Listener' -SelectorSet $selector -ValueSet $values -ErrorAction Stop}catch{$duplicateRejected=$true;$duplicateError=$_.ToString()}
    ...
    if($created){try {Microsoft.WSMan.Management\Remove-WSManInstance -ResourceURI 'winrm/config/Listener' -SelectorSet $selector -ErrorAction Stop}catch{$cleanupErrors+=$_.ToString()}}
