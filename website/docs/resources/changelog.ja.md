...
- 既存の監査専用ポリシーに対する `applocker-probe` の計画と固定 EXE プローブを追加。AppLocker イベントの厳密な照合、保護された証拠、ハッシュと変更検出を提供します。ポリシー・サービス・チャネルの変更や Sigma 検証済み判定は行いません。使い捨て Windows CI で既存インポーターと実際の 8003 を検証します。 (#423) (@Shirofune-Security)
