...
function Add-Rule {
    ...
    $detectRule = {
        function Search-DetectableEvents {
            ...
            $target = $event | where { $event.ProviderName -eq "Security" -and ($event.id -eq 4674 -and $event.message -match "C:\WINDOWS\SYSTEM32\SERVICES.EXE" -and $event.message -match "write_dac") }
            if ($target) {
                Write-Host
