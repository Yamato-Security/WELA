...
function Add-Rule {
    ...
    $detectRule = {
        function Search-DetectableEvents {
            ...
            $results = @();
            $results += Get-WinEvent -LogName Security | where { ($_.ID -eq "4799" -and $_.message -match "TargetUserName.*Administr.*" -and $_.message -match "CallerProcessName.*.*\\checkadmin.exe") } | select TimeCreated, Id, RecordId, ProcessId, MachineName, Message;
