...
The native observer uses local `EvtSubscribe` with `EvtSubscribeToFutureEvents | EvtSubscribeStrict` and a C# callback. No PowerShell script executes on the native callback thread. Once registration returns, a `Stopwatch` starts the observation window. Callbacks arriving before that window are excluded and counted separately. Each retained event records its monotonic callback-entry offset, original rendered XML and a native bookmark. The interval ends at the requested monotonic deadline; registration and shutdown/serialization are outside the rate denominator. UTC start/completion values aid correlation but do not replace the monotonic clock.
